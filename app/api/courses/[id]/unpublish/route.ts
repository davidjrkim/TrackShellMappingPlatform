...
export async function POST(_req: Request, { params }: { params: { id: string } }) {
  ...
  if (course.status !== 'published') {
    return NextResponse.json(
      { error: `Cannot unpublish course in status "${course.status}" — must be published` },
      { status: 409 },
    )
  }

  const affected = await setCourseStatus(params.id, session.user.orgId, ['published'], 'reviewed')
  if (affected === 0) {
