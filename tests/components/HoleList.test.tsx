...
describe('orderHolesForReview', () => {
  ...
  it('flags with needs_review sort to top by ascending confidence', () => {
    const holes = [
      hole({ hole_number: 1, confidence: 0.95 }),
      hole({ hole_number: 2, confidence: 0.55, needs_review: true }),
      hole({ hole_number: 3, confidence: 0.88 }),
      hole({ hole_number: 4, confidence: 0.40, needs_review: true }),
      hole({ hole_number: 5, confidence: 0.60, needs_review: true }),
    ]
  ...
  it('confirmed holes appear below flagged and in hole-number order', () => {
    const holes = [
      ...
      hole({ hole_number: 2, needs_review: true, confidence: 0.5 }),
  ...
  it('flagged holes that were already confirmed drop out of the flagged group', () => {
    const holes = [
      hole({ hole_number: 1, needs_review: true, confirmed: true, confidence: 0.3 }),
      hole({ hole_number: 2, needs_review: true, confirmed: false, confidence: 0.6 }),
    ]
    const out = orderHolesForReview(holes)
  ...
  it('ties in confidence fall back to hole number', () => {
    const holes = [
      hole({ hole_number: 5, needs_review: true, confidence: 0.5 }),
      hole({ hole_number: 2, needs_review: true, confidence: 0.5 }),
    ]
    const out = orderHolesForReview(holes)
...
describe('<HoleList />', () => {
  ...
  const holes = [
    hole({ hole_number: 1, confirmed: true }),
    hole({ hole_number: 2, needs_review: true, confidence: 0.4 }),
    hole({ hole_number: 3, confirmed: true }),
    hole({ hole_number: 4, needs_review: true, confidence: 0.7 }),
    hole({ hole_number: 5 }),
  ]
